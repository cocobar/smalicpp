			...
			if (strCmd.Find("nop") == 0) {
				...
				if (listI.size() == 3) {
					...
					if (strCmd.Find("const/4") == 0) {						//const/4 vx, lit4					存入4位常量到vx。
						...
						strType = this->pClass->GetCppTypeFromJava(CString("Ljava/lang/String;"));
						it->strCppExpression = listI[2];
					}
						...
						strType = this->pClass->GetCppTypeFromJava(listI[2]);
				...
				if (listI.size() == 3) {
					...
					CString strType = this->pClass->GetCppTypeFromJava(listI[2]);
					CString strInputR0 = listI[1];
					CString strExpression = SegmentListRetrodictGetExpression(listInstIndex, i, strInputR0, nInstIndex);
				...
				if (strCmd.Find("new-instance") == 0)
				{
					...
					if (listI.size() == 4) {
						...
						CString strType = this->pClass->GetCppTypeFromJava(listI[3]);
				...
				if (listI.size() == 4) {
					...
					if (strClassName != this->pClass->strClassName) {
						pClass->InsertExternClassField(listI[3], false);
					}
				}
				else {
					printf("%s 指令解析有错误\n", strInstString);
				...
				if (listI.size() == 3) {
					...
					if (strClassName != this->pClass->strClassName) {
						pClass->InsertExternClassField(listI[2], true);
					}
				...
				if (listI.size() == 3) {
					...
					if (strClassName != this->pClass->strClassName) {
						...
						if (strInstString.Find("invoke-static") == 0) {
						...
						// 放入队列
						if (strInstString.Find("invoke-static") == 0) {
							pClass->InsertExternClassMethod(listI[2], true);	// 静态的
						}
						else {
							pClass->InsertExternClassMethod(listI[2], false);	// 动态的
						}
					}
					//===============================================================
							...
							if (it->GetTotalSizeOfParam(listParamType) == (listReg.size() - 1)) {
								if (nIndexParam == 0) {
									...
									if ((strFunctionName.Compare("<init>") == 0) && (strExpression.Find("new ") == 0)) {
										...
										if (strCmd.Find("invoke-super") == 0) {
											strCppTotalExpression = this->strSuperClass + CString("::") + strFunctionName + CString("(");
										}
										else {
											if (strExpression == CString("this")) {
