...
void CJavaMethod::CreateSegmentList(std::vector<CString> listMethodInst)
{
		...
		if ((listI[0].Compare(".catch") == 0) || (listI[0].Compare(".catchall") == 0)) {
			...
			if (nFindTryCatchIndex >= 0) {		// 已经存在了
				if ((listI[0].Compare(".catch") == 0)) {
					...
					CString strType = this->pClass->GetCppTypeFromJava(listI[1].Left(nFindDotPost).Trim());
	...
	for (it = listMethodInst.begin(); it != listMethodInst.end(); it++, nLineNumber++) {
		...
		if (strLine.Find(".method") == 0) {
			...
			// 下面构造H文件中的函数
			CString strCppFunction;
			// 这里是一个构造函数
			if (this->strMethodName.Find("<init>") == 0) {
				// 插入析构函数
				//this->listCppStringCode
				strCppFunction.Format("~%s()", this->strClassName);
				pClass->listPublicMemberList.push_back(strCppFunction);

				strCppFunction.Format("%s(", this->strClassName);
			}
			else {
				strCppFunction.Format("%s %s(", this->strReturnType, this->strMethodName);
			}
			...
			for (unsigned int i = nStartParamIndex; i < this->listInputParam.size(); i++) {
				strCppFunction += (this->listInputParam[i].strTypeName + CString(" ") + this->listInputParam[i].strValueName);
				if ((i + 1) != this->listInputParam.size()) {
					strCppFunction += CString(",");
				}
			}
			strCppFunction += CString(")");

			if (find(listMethodAttr.begin(), listMethodAttr.end(), CString("static")) != listMethodAttr.end()){
				strCppFunction = (CString("static ") + strCppFunction);
			}

			// 在CJava结构体上填表
			if (find(listMethodAttr.begin(), listMethodAttr.end(), CString("public")) != listMethodAttr.end()){
				pClass->listPublicMemberList.push_back(strCppFunction);
			}
			else if (find(listMethodAttr.begin(), listMethodAttr.end(), CString("private")) != listMethodAttr.end()){
				pClass->listPrivateMemberList.push_back(strCppFunction);
			}
			else if (find(listMethodAttr.begin(), listMethodAttr.end(), CString("protected")) != listMethodAttr.end()){
				pClass->listProtectMemberList.push_back(strCppFunction);
			}
			else { // 没有属性列表的，都算作私有函数
				pClass->listPrivateMemberList.push_back(strCppFunction);
			}
		}
