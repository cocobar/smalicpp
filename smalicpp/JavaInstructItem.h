...
public:
	...
	// 从函数调用的汇编上获得完整参数和类型
	CString GetClassNameFromInstInvoke(CString strFunction);
	std::vector<CString> GetSignatureFromInstInvoke(CString strFunction);
	CString GetReturnTypeFromInstInvoke(CString strFunction);
	CString GetMethodNameFromInstInvoke(CString strFunction);

	// 从Field中获得完整的参数了类型
	CString GetClassNameFromInstIput(CString strParam);
	static CString GetFieldNameFromInstIput(CString strParam);
	CString GetFieldTypeFromInstIput(CString strParam);
